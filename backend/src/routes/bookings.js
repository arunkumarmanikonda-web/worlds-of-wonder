...
router.post('/',
  ...
  async (req, res) => {
    ...
    const BOOKING_WINDOW_DAYS = parseInt(process.env.BOOKING_WINDOW_DAYS || '365', 10);
    const maxDate = new Date(today.getTime() + BOOKING_WINDOW_DAYS * 86400000);
    if (visitDate > maxDate) {
      return res.status(400).json({ error: 'Booking window is ' + BOOKING_WINDOW_DAYS + ' days in advance' });
    }
      ...
      const result = await db.transaction(async (client) => {
        const { rows } = await client.query(
          ...
          [
            ...
            pricing.offer_code || null, pricing.discount || 0, b.payment_mode,
...
router.get('/',
  ...
  async (req, res) => {
    ...
    const filters = [];
    const params  = [];
    if (req.query.park)   { params.push(req.query.park);   filters.push('park = $' + params.length); }
    if (req.query.date)   { params.push(req.query.date);   filters.push('visit_date = $' + params.length); }
    if (req.query.status) { params.push(req.query.status); filters.push('booking_status = $' + params.length); }
    if (req.query.search) {
...
router.get('/:ref', async (req, res) => {
  try {
    const { rows } = await db.query('SELECT * FROM bookings WHERE booking_ref = $1', [req.params.ref]);
    if (!rows.length) return res.status(404).json({ error: 'Booking not found' });
    const booking = rows[0];
    if (booking.booking_status === 'confirmed' && !booking.qr_code_data) {
      const { dataUri } = await generateTicketQR(booking);
      await db.query('UPDATE bookings SET qr_code_data=$1 WHERE booking_ref=$2', [dataUri, req.params.ref]);
...
router.patch('/:ref',
  ...
  async (req, res) => {
    ...
    const sql = 'UPDATE bookings SET ' + updates.join(', ') + ', updated_at=$' + (params.length-1) + ' WHERE booking_ref=$' + params.length + ' RETURNING *';
    try {
...
router.delete('/:ref',
  ...
  async (req, res) => {
    try {
      const { rows } = await db.query(
        "UPDATE bookings SET booking_status='cancelled', updated_at=NOW() WHERE booking_ref=$1 RETURNING booking_ref, booking_status",
        [req.params.ref]
      );
      if (!rows.length) return res.status(404).json({ error: 'Booking not found' });
